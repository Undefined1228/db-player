...
export async function getSchemaObjects(connectionId: number, schemaName: string): Promise<SchemaObjects> {
  ...
  try {
    ...
    for (const r of indexesResult.rows) {
      ...
      indexesByTable.get(t)!.push(r.indexname as string)
    }
  ...
  switch (conn.dbType) {
    ...
    case 'mysql': {
      ...
      try {
        ...
        runningQueryMap.set(connectionId, { dbType: 'mysql', mysqlConnId: connection.threadId ?? undefined })
...
export async function cancelQuery(connectionId: number): Promise<{ ok: boolean }> {
  ...
  } else if (info.dbType === 'mysql' && info.mysqlConnId !== undefined) {
